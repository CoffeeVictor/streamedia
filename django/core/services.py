        ...
        with transaction.atomic():
            ...
            if video_media.status == VideoMedia.Status.UPLOAD_STARTED:
                self.storage.storage_chunk(
                    video_media.path, chunk_index, chunk)
                return

            if video_media.status == VideoMedia.Status.PROCESSING_FINISHED:
                video_media.path = directory
                video.is_published = False
                video.save()

            video_media.status = VideoMedia.Status.UPLOAD_STARTED
            video_media.save()
            self.storage.storage_chunk(video_media.path, chunk_index, chunk)
    ...
    def finalize_upload(self, video_id: int, total_chunks) -> None:
        ...
        try:
            ...
            is_chunks_valid = self.__validate_chunks(
                video.video_media.path, total_chunks)
        ...
        for filename in os.listdir(source_path):
            ...
            if os.path.isfile(file_path):
                try:
                    ...
                    print(
                        f"File {source_path}/{filename} moved to {dest_path}.")
